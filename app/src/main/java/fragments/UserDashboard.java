...
public class UserDashboard extends Fragment {
    ...
    private void initialize() {
        ...
        ProgressAdapter adapter = new ProgressAdapter(getContext(), list, 3);
        ...
        String userUniqueKey = FirebaseAuth.getInstance().getCurrentUser().getEmail()
                .replace( "@","-" )
                .replace( ".","-" );
        if (userUniqueKey != null) {
            DatabaseReference courseDatabase = FirebaseDatabase.getInstance().getReference()
                    .child("Progress").child(userUniqueKey);
